...
#### Each tool is invoked with minimal valid input:
    ...
    read_file_metadata	               Returns metadata, 
                                       rejects sensitive
    compute_hash	                   Computes SHA256, rejects >2GB
    move_file	                       Moves file within scope
    delete_file	                       Requires HITL, 
                                       rejects sensitive
    create_folder	                   Creates folder inside scope
    compress_files	                   Creates ZIP, skips sensitive
    write_log	                       Writes redacted audit entry
    read_log	                       Returns sanitized entries
    move_to_authenticated_folder	   Sensitive → authenticated only
