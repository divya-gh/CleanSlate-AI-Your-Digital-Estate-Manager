...
| `folder_scope_policy.dry_run` over `action_plan.dry_run` | `ActionPlan.dry_run` defaults to `True` always (planning is always a dry run). The *execution* dry_run flag must come from the policy. |
