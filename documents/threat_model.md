...
| **I** | Content Leakage | Mitigated by metadata-only architecture + Semgrep rules. | Implemented | Low (Zero file content read or uploaded) |
| **R** | Repudiation | Mitigated by structured audit logging + HITL boundaries. | Implemented | Low (Forensic append-only UTC logging) |
...
## 7. STRIDE Updates (Metadata-Only & Audit Logging)

This section updates the threat model to reflect the new metadata-only architecture, structured audit logging, and Semgrep static safety gates.

### Information Disclosure
- **Mitigation**: Classification (`ClassificationNode`) and sensitivity detection (`SensitiveDetectionNode`) are now strictly metadata-only. All preview, PDF text extraction, and file content-reading logic have been completely removed. LLM generation queries strictly receive metadata parameters (derived heading, extension, parent directory, size, path depth, and timestamps).
- **Semgrep Gates**: Enforced by static security rules:
  - `no-file-content-reading`: Blocks file reads.
  - `no-content-upload-to-llm`: Blocks passing raw content/previews to Gemini models.

### Repudiation
- **Mitigation**: Integrated a centralized structured audit logger (`app/security/audit_logger.py`).
- **Forensic Integrity**: Written in append-only JSONL format using UTC timestamps.
- **Redaction Rules**: Absolute paths are completely masked. Non-sensitive files show only `parent_folder/filename`, and sensitive files are masked to `"<sensitive file>"`.
- **Node Coverage**: Audit logs are generated by `ExecutionNode`, `HITLApprovalNode`, `RollbackNode`, `WeeklyOrganizerNode`, and `SummaryNode`.

### Denial of Service
- **Mitigation**: Transitioning to metadata-only processing reduces I/O read operations, lowering memory overhead and improving workspace traversal resilience.

### Elevation of Privilege
- **Mitigation**: Enforced that all destructive/cleanup actions must pass through the authorized `ExecutionNode` boundary.
- **Semgrep Gates**: Complies with the `file-ops-must-use-folder-scope` rule, verifying folder scope permissions before executing system changes.
